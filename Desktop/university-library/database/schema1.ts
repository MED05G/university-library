...
export const accountRequests = pgTable('account_requests', {
  ...
  address: text('address'),
  departmentId: uuid('department_id').references(() => departments.id),
  universityCardUrl: text('university_card_url'),
  requestDate: timestamp('request_date', { withTimezone: true }).defaultNow(),
  status: text('status').notNull().default('pending'),
  ...
  statusCheck: check('chk_account_request_status', sql`${table.status} IN ('pending', 'approved', 'rejected')`),
  rejectionCheck: check('chk_account_request_rejection', sql`
    (${table.status} = 'rejected' AND ${table.rejectionReason} IS NOT NULL) OR
    ${table.status} != 'rejected'
  `),
  reviewCheck: check('chk_account_request_review', sql`
    (${table.status} IN ('approved', 'rejected') AND ${table.reviewedBy} IS NOT NULL AND ${table.reviewedAt} IS NOT NULL) OR
    ${table.status} = 'pending'
  `),
  
  // Indexes
  statusIdx: index('idx_account_requests_status').on(table.status),
  emailIdx: index('idx_account_requests_email').on(table.email),
  reviewerIdx: index('idx_account_requests_reviewer').on(table.reviewedBy),
  pendingIdx: index('idx_account_requests_pending').on(table.id).where(sql`${table.status} = 'pending'`),
}));
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  reviewedAccountRequests: many(accountRequests, { relationName: 'reviewer' }),
  approvedAccountRequests: many(accountRequests, { relationName: 'approver' }),
}));

export const accountRequestsRelations = relations(accountRequests, ({ one }) => ({
  department: one(departments, {
    fields: [accountRequests.departmentId],
    references: [departments.id],
  }),
  reviewer: one(users, {
  ...
  approvedUser: one(users, {
    fields: [accountRequests.approvedUserId],
    references: [users.id],
